<?php
/* ============================================================
 *  organizer/organizer_admin_chat.php
 *  Organizer ↔ Admin Messenger (organizer's view)
 * ============================================================ */
session_start();
...
$orgLogoData = '';
$orgMime     = 'image/jpeg';
if (!empty($ctx['org_logo'])) {
    $fi = new finfo(FILEINFO_MIME_TYPE);
    $dt = $fi->buffer($ctx['org_logo']);
    if ($dt && strpos($dt, 'image/') === 0) $orgMime = $dt;
    $orgLogoData = base64_encode($ctx['org_logo']);
    $hasOrgLogo  = true;
}
?>
<!DOCTYPE html>
...
<div class="lg:ml-64 flex flex-col min-h-screen">
    ...
    <header class="sticky top-0 z-30 bg-white/90 dark:bg-gray-800/90
        ...
        <div class="flex items-center gap-3">
            ...
            <i class="fas fa-user-shield text-brand-500 text-sm"></i>
            <span class="font-semibold text-gray-900 dark:text-white">Admin Messages</span>

            <span class="hidden sm:inline-flex items-center gap-1 text-xs px-2.5 py-1 rounded-full
                         bg-brand-100 dark:bg-brand-900/40 text-brand-700 dark:text-brand-300">
    ...
    <div class="chat-shell relative">
                ...
                <div class="relative">
                    ...
                    <input id="searchInput" type="search"
                           class="w-full pl-8 pr-3 py-2 text-sm rounded-lg
                                  ...
                                  placeholder-gray-400 focus:border-brand-400 transition-colors"
        ...
        <!-- RIGHT: thread panel -->
        <div class="chat-thread-panel bg-gray-50 dark:bg-gray-900" id="threadPanel">
            ...
            <div id="threadEmpty" class="flex flex-col items-center justify-center flex-1 gap-3 text-gray-400 p-8">
                <div class="w-14 h-14 rounded-2xl bg-brand-100 dark:bg-brand-900/30 flex items-center justify-center">
                    <i class="fas fa-user-shield text-2xl text-brand-500"></i>
                </div>
                <p class="font-semibold text-gray-700 dark:text-gray-200 text-sm">Message an Administrator</p>
                <p class="text-xs text-center max-w-[200px] leading-relaxed">
                    Use <strong>New Message</strong> to contact an admin, or click an existing conversation.
                </p>
            </div>
                ...
                <div class="input-bar border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800">
                    ...
                    <textarea id="msgInput" rows="1"
                        class="chat-ta bg-gray-50 dark:bg-gray-700 border-gray-200 dark:border-gray-600
                               text-gray-800 dark:text-gray-100 placeholder-gray-400 focus:border-brand-400 outline-none"
...
<div id="unsendModal" onclick="handleUnsendBgClick(event)">
    <div class="unsend-modal-box">
        <div style="display:flex;align-items:flex-start;gap:.85rem;margin-bottom:.1rem;">
            ...
            <div>
                <p class="font-bold text-gray-900 dark:text-white" style="font-size:.97rem;">
                    Unsend this message?
                </p>
