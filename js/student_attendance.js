...
function applyDark(on) {
  ...
  ['sunIconM','sunIconD'].forEach(id  => { const e=document.getElementById(id); if(e) e.style.display=on?'block':'none'; });
  ['moonIconM','moonIconD'].forEach(id => { const e=document.getElementById(id); if(e) e.style.display=on?'none':'block'; });
}
  ...
  if (tbody) {
    if (!slice.length) {
      ...
      tbody.innerHTML = slice.map(r => `
        <tr>
          <td>
            <div style="font-weight:600;font-size:.84rem;color:var(--ink);max-width:200px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${r.title}</div>
            <div style="font-size:.7rem;color:var(--ink3);margin-top:.1rem;">${r.organizer}</div>
          </td>
          <td><span class="pill pill-accent">${r.type}</span></td>
          <td style="max-width:140px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;font-size:.8rem;" title="${r.venue}">${r.venue}</td>
          <td>${r.login  ? `<span class="time-pill time-in"><i data-lucide="log-in" style="width:10px;height:10px;"></i> ${fmtTime(r.login)}</span><br><span style="font-size:.68rem;color:var(--ink3);">${fmtDate(r.login)}</span>` : '<span style="color:var(--ink3);">—</span>'}</td>
          <td>${r.logout ? `<span class="time-pill time-out"><i data-lucide="log-out" style="width:10px;height:10px;"></i> ${fmtTime(r.logout)}</span><br><span style="font-size:.68rem;color:var(--ink3);">${fmtDate(r.logout)}</span>` : '<span style="color:var(--ink3);">—</span>'}</td>
    ...
    if (!slice.length) {
      ...
      mobCont.innerHTML = slice.map(r => `
        <div class="mob-card">
          ...
          <div class="mob-card-meta">${r.venue} · ${r.type}</div>
